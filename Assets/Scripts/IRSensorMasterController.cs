...
public class IRSensorMasterController : MonoBehaviour
{
    ...
    // Setup events for sending IR sensor data to m_IRSensorMasterController.DataReceivedHandler

    public delegate void OnAverageIRSignalReceived (int[] avgDistances);
    public event OnAverageIRSignalReceived onAverageIRSignalReceived;
    ...
    int m_numBytesToRead; // two bytes for each IR sensor distance reading; two IR sensors
    int m_numAccumReadBytes = 0;

    float m_Delay;
    public const int m_IRDistanceBytesCount = 4; // two bytes for each IR sensor distance reading; two IR sensors
...
//#endif
    ...
    //cf https://jonskeet.uk/csharp/readbinary.html
    //https://csharp.hotexamples.com/examples/System.IO.Ports/SerialDataReceivedEventArgs/-/php-serialdatareceivedeventargs-class-examples.html

    //public static void port_DataReceived(object sender, SerialDataReceivedEventArgs e)
    //{
    //    if (e.EventType == SerialData.Chars) // SerialData.Eof
    //    {
    //        receiveNow.Set();
    //    }
    //}
    ...
    //When using DataReceived event handlers, don’t assume you’ll receive an event for every byte received
    //(assuming you’re using the default ReceivedBytesThreshold of 1)
    //For example, if several bytes arrived at the serial port at the same time, 
    //you’d receive just one event, not one for each byte.
    //To get the raw bytes: The BytesToRead property tells you how many bytes are available.
    //Then you can call SerialPort.Read(buffer, offset, count) 
    //with a buffer big enough to hold the bytes (or call over several iterations).
    ...
    void DataReceivedHandler(object sender, SerialDataReceivedEventArgs e)
    {
        ...
        //{
        //    return;  //The eof char was received, do nothing and  return 
        ...
        //Here’s what Read actually does. If there are bytes available on the serial port, 
        //Read returns up to count bytes but will not block for the remaining bytes.
        //If there are no bytes available on the serial port, Read will block 
        //until at least one byte is available on the port, up until the ReadTimeout milliseconds have elapsed, 
        ...
        //https://docs.microsoft.com/en-us/dotnet/api/system.io.stream.readbyte?view=netframework-4.8

        int count = port.BytesToRead;
        int numOfBytesRead;

        Debug.Log("count in IRDataReceived=" + count);
        ...
        {  // the full number of bytes expected has arrived
            try
            {
                ...
                numOfBytesRead = port.Read(m_IRDistanceBytes, 0, count);
                m_numAccumReadBytes += numOfBytesRead;
            }
            catch (Exception ex)
            {
        ...
        { // more than needed has arrived, so only the needed part should be read
            int countNeeded = m_numBytesToRead - m_numAccumReadBytes;
            numOfBytesRead =  port.Read(m_IRDistanceBytes, 0, countNeeded);
        ...
        {  // m_numAccumReadBytes is less than m_IRDistanceBytesCount
            // return and wait for the next DataReceived event to read more bytes
    ...
    {
        // It is assumed that  four bytes will arrive from the serial port
        ...
        uint uintForLeftByte = (uint) m_IRDistanceBytes[0]; // copy a byte to an unsigned  int (== UInt32)
        uintForLeftByte = uintForLeftByte << 8; // shift 8 bits of the uint32 integer to the left
        uint uintForRightByte = m_IRDistanceBytes[1];

        m_averageDistances[0] = (int)(uintForLeftByte | uintForRightByte);

        uintForLeftByte = m_IRDistanceBytes[2];
        uintForLeftByte = uintForLeftByte << 8;
        uintForRightByte = m_IRDistanceBytes[3];

        m_averageDistances[1] = (int)(uintForLeftByte | uintForRightByte);

        Debug.Log("the first avg distance=" + m_averageDistances[0]);
        Debug.Log("the second  avg distance=" + m_averageDistances[1]);

        
        onAverageIRSignalReceived.Invoke(m_averageDistances);
