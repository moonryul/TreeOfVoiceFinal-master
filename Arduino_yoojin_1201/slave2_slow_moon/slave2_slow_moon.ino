...
ISR (SPI_STC_vect) { // SPI_STC_vect: invoked when data arrives:
  ...
  // EVEN IF THE INTERRUPT OCCURS, IT DOES NOT CHANGE THE BUFFER WHILE loop() is reading the buffer
  // It will cause the data loss. 
...
void loop() {
  ...
  { // get the bytes in the fully filled buffer
    ...
    for (int i = 0; i < NUMPIXELS1; i++)
    {
     ...
     // strip.setPixelColor(i, m_buffer[i * 3 + 2], m_buffer[i * 3 + 1], m_buffer[ i * 3 + 0] );
      Serial.print(i);
      Serial.print("th  ");
      Serial.print("  r:  ");
      Serial.print(m_buffer[i * 3 + 2]);
      Serial.print("  g:  ");

      Serial.print(m_buffer[i * 3 + 1]);
      Serial.print("  b:  ");
      Serial.println(m_buffer[i * 3 + 0]);
    }
